...
<div class="updated2">
    ...
    ** NOTE: ** Support is handled on our site: <a href="http://club.orbisius.com/support/" target="_blank" title="[new window]">http://club.orbisius.com/support/</a>
    ...
    <br/>
    <div>
        Download <a href="<?php echo $webweb_wp_like_gate_obj->get('plugin_url');?>doc/Orbisius_LikeGate_Documentation.pdf" target="_blank">
            Plugin Documentation
        </a> (173KB PDF) (Right Click and then Save Link As)
    </div>
</div>

<?php return ; ?>

<?php if ((preg_match('#win#si', PHP_OS) || $_SERVER['HTTP_HOST'] == 'localhost')) : ?>
    <p>
        <iframe src="<?php echo $webweb_wp_like_gate_obj->get('plugin_support_link');?>"
...
<?php else : ?>
<?php
$to = $webweb_wp_like_gate_obj->get('plugin_support_email');
...
$site = app_contact_get_var('site', $webweb_wp_like_gate_obj->get('site_url'));
...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        ...
        if ($status) {
            $contact_msg = $webweb_wp_like_gate_obj->message("Your message has been sent.", 1);
            // everything went fine let's clear the fields.
            $name = $phone = $email = $site = $subject = $message = '';
        } else {
            $contact_msg = $webweb_wp_like_gate_obj->message("Cannot send email. Please send your request directly to: $to");
        }
    } else {
        $contact_msg = $webweb_wp_like_gate_obj->message("<h3>Errors: </h3>" . join("<br/>\n", $errors));
    }
}
...
<p>If you want to reach us
    via email: <a href="mailto:<?php echo $webweb_wp_like_gate_obj->get('plugin_support_email');?>?subject=<?php echo urlencode($webweb_wp_like_gate_obj->get('plugin_id_str'));?>"><?php echo $webweb_wp_like_gate_obj->get('plugin_support_email');?></a>
    or phone: US: <a href="tel:1-716-514-8880">1-716-514-8880</a>, Canada: <a href="tel:1-647-478-6512">1-647-478-6512</a>
    <br/>Plugin's page: <a href="<?php echo $webweb_wp_like_gate_obj->get('plugin_home_page');?>" target="_blank"><?php echo $webweb_wp_like_gate_obj->get('plugin_home_page');?></a>
<?php if (0) : ?>
or via skype:
<!--
